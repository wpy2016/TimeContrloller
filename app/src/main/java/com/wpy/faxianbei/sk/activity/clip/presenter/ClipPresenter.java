    ...
    public void onActivityResult(Context mContext,int requestCode,int resultCode,Intent data){
        ...
        switch (requestCode) {
            ...
            case PHOTOCOMPLETEBYTAKE:
                ...
                mPath = temppath;
                //通过接口传递出去
                getViewInterface().clipComplete(FileUtil.getBitmapFormPath(mContext, temppath),temppath);
                break;
            case PHOTOCOMPLETEBYGALLERY:
                final String temppathgallery = data.getStringExtra("path");
                mPath = temppathgallery;
                //通过接口传递出去
                getViewInterface().clipComplete(FileUtil.getBitmapFormPath(mContext, temppathgallery),temppathgallery);
                break;
        }
    }
